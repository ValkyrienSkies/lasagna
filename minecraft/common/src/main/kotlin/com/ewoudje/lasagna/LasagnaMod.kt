...
import com.ewoudje.lasagna.chunkstorage.setSectionStorage
import com.ewoudje.lasagna.networking.LasagnaNetworking
import com.ewoudje.lasagna.networking.PlayerPacketTarget
import com.ewoudje.lasagna.networking.ServerPacketTarget
import com.ewoudje.lasagna.networking.defineSerialization
import com.ewoudje.lasagna.scaleblocks.ScaleBlocksView
...
object LasagnaMod {
    ...
    fun init() {
        ...
        LasagnaNetworking.packetServer { p: Ping, serverPlayer: ServerPlayer ->
            LasagnaNetworking.send(PlayerPacketTarget(serverPlayer),
                Ping()
            )
        }
            ...
            literal<CommandSourceStack>("lasagna").then(
                literal<CommandSourceStack>("debug").executes { ctx ->
                    ...
                    argument<CommandSourceStack, ItemInput>("item", ItemArgument.item()).executes { ctx ->
                        ...
                        val tags = getTags(
                            Registry.ITEM.getResourceKey(item.item).get()
                        )
                    ...
                    val tags = getTags(
                        Registry.ITEM.getResourceKey(item).get()
                    )
